	...
	private function callback_file($m)
	{
		...
		switch (strtolower($m[1]))
		{
			...
			case 'delete_link':
			{
				...
				' [<a href="' . Url::html_output(INDEX_FILE) . '?action=delete&amp;dir=' . rawurlencode($subdir)
			...
			{
				return (($this -> is_admin && !$this -> temp_item -> __get('is_parent_dir')) ?
				' [<a href="' . Url::html_output(INDEX_FILE) . '?action=rename&amp;dir=' . rawurlencode($subdir)
				. '&amp;filename=' . rawurlencode($this -> temp_item -> __get('filename'))
			...
			case 'edit_description_link':
			{
				...
				' [<a href="' . Url::html_output(INDEX_FILE) . '?action=edit_description&amp;dir='
			...
			case 'ftp_upload_link':
			{
				...
				return ' [<a href="' . Url::html_output(INDEX_FILE) . '?action=ftp&amp;dir='
