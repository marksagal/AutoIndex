...
class FileItem extends Item
{
	...
	public function __construct($parent_dir, $filename)
	{
		...
		$this -> link = Url::html_output(INDEX_FILE) . '?dir=' . Url::translate_uri(substr($this -> parent_dir, strlen($config -> __get('base_dir'))))
		. '&amp;file=' . Url::translate_uri($filename);
		if (THUMBNAIL_HEIGHT && in_array(self::ext($filename), array('png', 'jpg', 'jpeg', 'gif')))
		{
			$this -> thumb_link = ' <img src="' . Url::html_output(INDEX_FILE)
