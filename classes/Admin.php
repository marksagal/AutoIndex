	...
	public function action($action)
	{
		...
		switch (strtolower($action))
		{
			...
			case 'rename':
			{
				...
				. ')</span></p><form method="get" action="' . Url::html_output(INDEX_FILE)
			...
			case 'delete':
			{
				...
				. '<form method="get" action="' . Url::html_output(INDEX_FILE)
			...
			case 'add_user':
			{
				...
				. Url::html_output(INDEX_FILE) . '?action=add_user"><p>'
			...
			case 'change_password':
			{
				...
				. Url::html_output(INDEX_FILE) . '?action=change_password">
			...
			case 'change_user_level':
			{
				...
				. Url::html_output(INDEX_FILE) . '?action=change_user_level">
			...
			case 'del_user':
			{
				if (isset($_POST['username']))
				{
					...
					. '<form method="post" action="' . Url::html_output(INDEX_FILE) . '?action=del_user">'
				...
				. ':</p><form method="post" action="' . Url::html_output(INDEX_FILE)
			...
			case 'edit_description':
			{
				if (isset($_GET['filename']))
				{
					...
					else
					{
						...
						. ' <em>' . Url::html_output($_GET['filename'])
						. '</em>:</p><form method="get" action="' . Url::html_output(INDEX_FILE)
			...
			case 'edit_hidden':
			{
				...
				. Url::html_output(INDEX_FILE) . '"><p><input type="hidden" name="action" value="edit_hidden" />'
				...
				. Url::html_output(INDEX_FILE) . '"><p><select name="remove">';
				foreach ($hidden_list as $hid)
				{
			...
			case 'edit_banned':
			{
				...
				. Url::html_output(INDEX_FILE) . '"><p><input type="hidden" name="action" value="edit_banned" />'
				. '<input type="text" name="add" size="40" /> <input type="submit" value="'
				. $words -> __get('add') . '" /></p></form>';
			...
			case 'create_dir':
			{
				...
				else
				{
					...
					. Url::html_output(INDEX_FILE) . '"><p><input type="hidden" name="action" value="create_dir" />'
			...
			case 'ftp':
			{
				...
				if (isset($_GET['ftp_logout']))
				{
					...
					. Url::html_output(INDEX_FILE) . '?dir='
				...
				else if (isset($_SESSION['ftp']))
				{
					...
					global $words;
					$text = '<ul><li><a href="' . Url::html_output(INDEX_FILE)
				...
				else
				{
					...
					<p><a class="autoindex_a" href="' . Url::html_output(INDEX_FILE)
...
<p>
	<a href="' . Url::html_output(INDEX_FILE) . '?action=config" class="autoindex_a">'
	. $words -> __get('reconfigure script') . '</a>
</p>
<p>
	<a href="' . Url::html_output(INDEX_FILE) . '?action=edit_hidden" class="autoindex_a">'
	. $words -> __get('edit list of hidden files') . '</a>
	<br /><a href="' . Url::html_output(INDEX_FILE) . '?action=edit_banned" class="autoindex_a">'
	. $words -> __get('edit ban list') . '</a>
</p>
<p>
	<a href="' . Url::html_output(INDEX_FILE) . '?action=create_dir&amp;dir=' . rawurlencode($subdir)
	. '" class="autoindex_a">' . $words -> __get('create new directory in this folder')
	. '</a><br /><a href="' . Url::html_output(INDEX_FILE) . '?action=copy_url&amp;dir='
	. $subdir . '" class="autoindex_a">' . $words -> __get('copy url') . '</a>
</p>
<p>
	<a href="' . Url::html_output(INDEX_FILE) . '?action=view_log" class="autoindex_a">'
	. $words -> __get('view entries from log file') . '</a>
	<br /><a href="' . Url::html_output(INDEX_FILE) . '?action=stats" class="autoindex_a">'
	. $words -> __get('view statistics from log file') . '</a>
</p>
<p>
	<a href="' . Url::html_output(INDEX_FILE) . '?action=add_user" class="autoindex_a">'
	. $words -> __get('add new user') . '</a>
	<br /><a href="' . Url::html_output(INDEX_FILE) . '?action=del_user" class="autoindex_a">'
	. $words -> __get('delete user') . '</a>
	<br /><a href="' . Url::html_output(INDEX_FILE) . '?action=change_user_level" class="autoindex_a">
	Change a user\'s level</a>
</p>';
		//MODERATOR and ADMIN accounts
		if ($this -> level >= MODERATOR) $str .= '
<p>
	<a href="' . Url::html_output(INDEX_FILE) . '?action=change_password" class="autoindex_a">
	Change your password</a>
</p>
<p>
	<a href="' . Url::html_output(INDEX_FILE) . '?action=ftp&amp;dir=' . rawurlencode($subdir)
